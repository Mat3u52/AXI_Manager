...
class FormValidation:
    ...
    def validatorItem(self, item, itemAmount):
        ...
        if item != '' and itemAmount != '' and \
                type(itemAmount) is not str and int(itemAmount) > 0:
    ...
    def validatorViTrox(self, prog="", test=0, linecapa="NONE", epi="NONE", baan1="NONE", comments="",
                        flagItemStatus=False, itemAmount=0):

        self.flagValidatorViTrox = False
        self.flagInitStatus = flagItemStatus
        if self.flagInitStatus is True and\
                prog != '' and \
                (type(test) is not str and int(test) > 0) and\
    ...
    def validator5DX(self, prog="", test=0, linecapa="NONE", epi="NONE", baan1="NONE", comments="",
                     flagItemStatus=False, itemAmount=0, alignTime=0, laserTime=0, thicknessTime=0):

        self.flagValidator5DX = False
        self.flagInitStatus = flagItemStatus
        if self.flagInitStatus is True and \
                prog != '' and \
            ...
            if (self._computeUPH(int(self.totalTime), 85, int(self.itemAmount)) > 0) and \
                ...
                self.uph95 = self._computeUPH(int(self.totalTime), 95, int(self.itemAmount))
                self.uph95Time = self._convertUPHToTime(self.uph95, self.itemAmount)
                self.flagValidator5DX = True

                messagebox.showwarning("5dx OK")
    ...
    def readyToInsert(self, totalTestV849, totalTestV817):
    ...
    def cleanUpViTrox(self, ei0, ei1, ci2, ci3, ci4, ei5):
    ...
    def cleanUp5DX(self, ei0, ei1, ci2, ci3, ci4, ei5, ei6=0, ei7=0, ei8=0):
        if (self.flagValidator5DX is True) \
                and ei0 and ei1 and ci2 and ci3 and ci4 and ei5:
            self.flagInitStatus = False
            self.ei0 = ei0
            self.ei1 = ei1
